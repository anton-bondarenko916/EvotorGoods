...
class EvotorRepository(private val goodDatabase: GoodDatabase, private val api: Api) {

    suspend fun getGoods(shopUUID: String): List<Good> {
        try {
            val response = api.getGoods(shopUUID)
            if (response.isSuccessful) {
                goodDatabase.goodDao().insertItems(response.body()?.items?.map { it.toRoomGood() } ?: listOf())
                return response.body()?.items?.map { it.toGood() } ?: throw RuntimeException("goods is null")
            } else {
                throw RuntimeException("some shit")
            }
        } catch (exception: Exception) {
            val itemsFromDatabase = goodDatabase.goodDao().getItems().map { it.toGood() }
